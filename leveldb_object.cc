...
public:
    ...
    int Compare(const leveldb::Slice& a, const leveldb::Slice& b) const
    {
        ...
        PyObject* a_ = PyByteArray_FromStringAndSize(a.data(), a.size());
        PyObject* b_ = PyByteArray_FromStringAndSize(b.data(), b.size());
...
static PyObject* PyLevelDB_GetStats(PyLevelDB* self)
{
    std::string value;
...
static PyMethodDef PyLevelDB_methods[] = {
    ...
    {(char*)"GetStats",         (PyCFunction)PyLevelDB_GetStats,       METH_NOARGS,                    (char*)"get a mapping of all DB statistics"},
...
static int pyleveldb_str_eq(PyObject* p, const char* s)
{
    ...
    if (PyUnicode_Check(p)) {
        // Py_ssize_t len;
        // wchar_t* wsz = PyUnicode_AsWideCharString(p, &len);

        // size_t i = 0;
        // while (s[i] && wsz[i] && (int)s[i] == (int)wsz[i])
        //     i++;
        // return ((int)s[i] == (int)wsz[i]);

        PyObject* so = PyUnicode_FromStringAndSize(s, strlen(s));
        if (so == 0) return 0;

        int r = PyObject_RichCompareBool(p, so, Py_EQ);
        Py_DECREF(so);
        return r;
    }
